...
| `pdf_url` | arXiv > Semantic Scholar openAccessPdf > 其他 |
| `abstract` | Semantic Scholar > arXiv > ACM/IEEE |
| `venue` | ACM DL > IEEE > Semantic Scholar > arXiv |
| `doi` | ACM DL > IEEE > Semantic Scholar > arXiv |
...
| 标准字段 | arXiv XML | Semantic Scholar | PubMed esummary docsum | ACM JSON-LD | IEEE API |
|---------|-----------|-----------------|---------------|-------------|---------|
| title | `<title>` | `title` | `title` | `name` | `title` |
| authors | `<author><name>` | `authors[].name` | `authors[].name`（常为 `LastName Initials`） | `author[].name` | `authors.authors[].full_name` |
| year | `<published>`[0:4] | `year` | `pubdate`[0:4] | `datePublished`[0:4] | `publication_year` |
| doi | `<arxiv:doi>` | `externalIds.DOI` | `articleids[doi].value` | `@id`（DOI URL） | `doi` |
| arxiv_id | `<id>`（末段） | `externalIds.ArXiv` | - | - | - |
| abstract | `<summary>` | `abstract` | （需 efetch） | `description` | `abstract` |
| citation_count | - | `citationCount` | - | - | `citing_paper_count` |
| pdf_url | `<link type=pdf>` | `openAccessPdf.url` | - | - | `pdf_url` |
